...
class ProductControllerTest {
    ...
    @Test
    @DisplayName("전체 상품 조회 Controller 테스트")
    void testSelectProductByName() throws Exception   {
        int page = 0;
        int size = 1;
        ...
        when(productService.selectProductByName(mockProducts.getName(),page, size)).thenReturn(new ProductSelectPagingResponseDto(
                content.stream().map(ProductSelectResponseDto::new).collect(Collectors.toList()),
                productPage.getNumber(),
                productPage.getSize())
        );
        ...
        //when & then: 예상되는 결과 검증
        mockMvc.perform(get("/frankit/product-manage/product/name")
                        .param("name",String.valueOf(mockProducts.getName()))
                        .param("pageIndex",String.valueOf(page))
                        .param("pageSize",String.valueOf(size))
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                ...
                .andDo(
                        document("product/name", //여기이름이 스니펫 폴더 이름
                                queryParameters(  // 실제 요청 본문 필드 설명
        ...
        verify(productService, times(1)).selectProductByName(mockProducts.getName(),page,size);
    }
